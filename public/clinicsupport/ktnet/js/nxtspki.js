...
var nxTSPKIObj = (function(parentObj, $) {
  ...
  _toolkit._getResultTimer = new nxTSUtil.timer(nxTSConfig.getResultInterval, function() {
    // UI 스크립트 오류 대응. 한번 발생하면 주기적으로 계속 발생함. 원인확인용도
    try {
      nxTSPKIObj.getResult(nxts_get_result_complete_callback).invoke();
    } catch (e) {
      console.log("nxtspki error...");
    }
  });
...
var nxTSPKI = (function(parentObj, $) {
  ...
  _toolkit.callWithInit = function(name, fn, args) {
    ...
    if (nxTSSession.isInit() == false) nxTSCommon.init({ versionCheck: [nxTSConfig.TSTOOLKIT], installMessage: nxTSPKIConfig.installMessage, installPage: nxTSPKIConfig.installPage }, initSuccess);
    else initSuccess({ code: nxTSError.res_success });
  };
    ...
    setTimeout(function() {
      if (nxTSSession.isInit() == false) {
        nxTSCommon.init(
          ...
          function(res) {
            if (res.code == nxTSError.res_success) nxTSPKI.callPostInit();
          },
        );
      } else {
